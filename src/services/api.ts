...
const injectedRtkApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    createBanner: build.mutation<CreateBannerApiResponse, CreateBannerApiArg>({
      query: (queryArg) => ({ url: `/Banners`, method: 'POST', body: queryArg.bannerCreateDto }),
    }),
    editBanner: build.mutation<EditBannerApiResponse, EditBannerApiArg>({
      query: (queryArg) => ({ url: `/Banners`, method: 'PUT', body: queryArg.bannerEditDto }),
    }),
...
export type SearchPostsApiResponse = /** status 200 Success */ PostDto[];
...
export type SearchPublicPostsApiResponse = /** status 200 Success */ PostDto[];
